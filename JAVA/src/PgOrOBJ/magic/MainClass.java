...
//		}
		...
		contenedorCartas = new ContenedorCartas();
		for (int i= 0; i < cartas.length; i++) {
			contenedorCartas.addCarta(cartas[i]);
		}
		ContenedorJugadoresCartas contenedor_jugadores_cartas = new ContenedorCartas();
		contenedor_jugadores_cartas.addJugador(jugadores_cartas[6]);
		contenedor_jugadores_cartas.addJugador(jugadores_cartas[7]);
		contenedor_jugadores_cartas.addJugador(jugadores_cartas[8]);
		contenedor_jugadores_cartas.addJugador(jugadores_cartas[9]);
		contenedor_jugadores_cartas.addJugador(jugadores_cartas[10]);

		
		
		PartidaCartas pc = new PartidaCartas(contenedorCartas,contenedor_jugadores_cartas);
		pc.jugar();
	}
}
...
class PartidaCartas {
	...
	int reduccion, acierto;
	private Carta carta_atacante;
	private Carta carta_defensora;
	private int recuperacion_mana;
	...
	public void jugar() {
		...
		jugadores_turno = contenedor_jugadores_partida.getJugadoresAleatorios();
		turno = ((int) (10*Math.random()))%2;
		while (jugadores_turno[0].getVida() > && jugadores_turno[1])
		carta_atacante = jugadores_turno[turno].getCartaAleatoria();
		carta_defensora = jugadores_turno[(turno-1)%2].getCartaAleatoria();
		recuperacion_mana = (int) (3*Math.random() + 1);

		
		if (jugadores_turno[turno].getMana() >= carta_atacante.getCosteMana();{

			reduccion = (int) (carta_defensora.getDenfensa()*10)/100;
			
			dano = carta_atacante.getAtaque() - (int) (carta_atacante.getAtaque()*reduccion/10);
			...
			jugadores_turno[turno].setMana(jugadores_turno[turno].getMana() + carta_atacante.getCosteMana() + recuperacion);
			if (acierto) {
				jugadores_turno[(turno+1)%2].setVida(jugadores_turno[(turno+1)%2].getVida()-daño);
			}
		}else {
			jugadores_turno[turno].setMana(jugadores_turno[turno].getMana()+recuperacion_mana);
		}
		turno ++;
		
		
		if (jugadores.turno[0].getVida()>0) {
			System.out.println("Jugador0")
		}
	}
}
